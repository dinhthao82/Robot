...
//|             AutoProject V2 | v1.12 — Optimized (P1 Top-1)       |
//+------------------------------------------------------------------+
...
input double GridStep        = 2.0;      // Giá giảm X USD → thêm BUY tiếp theo  [P1 opt: 2.0]
input int    MaxPositions    = 15;       // Tối đa bao nhiêu BUY / cycle            [P1 opt: 15]
input double TakeProfit_Pct  = 20.0;    // Đóng tất cả khi floating >= X% balance  [P1 opt: 20%]
...
input double MaxDDPct        = 20.0;    // Max daily drawdown % → dừng hôm nay    [P1 opt: 20%]
