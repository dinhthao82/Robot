//+------------------------------------------------------------------+
//|                              NewBotGold_V3_TwoWay.mq5            |
//|             BUY + SELL Martingale Grid — XAUUSD — DAY TRADING   |
//|             AutoProject V2 | v3.00                               |
//|                                                                  |
//|  V3 vs V2:                                                       |
//|  • Thêm SELL side — hai chiều BUY và SELL chạy độc lập song song|
...
input double BalancePer      = 1000.0;
input double MinLot          = 0.01;
input double MaxLot          = 2.00;

//=== GRID STRATEGY ===
input group "=== GRID STRATEGY ==="
input double GridStep        = 2.0;
input int    MaxPositions    = 15;
input double TakeProfit_Pct  = 20.0;

//=== SPREAD ===
input group "=== SPREAD ==="
input int    MaxSpread_Pts   = 300;
...
input int    ServerGMT       = 3;

//=== DAY TRADING ===
input group "=== DAY TRADING ==="
input int    ForceCloseMin   = 5;
input int    NoNewOrderMin   = 10;

//=== RISK MANAGEMENT ===
input group "=== RISK MANAGEMENT ==="
input double MaxDDPct        = 20.0;
input double StopBotPct      = 50.0;

//=== MONEY MANAGEMENT ===
input group "=== MONEY MANAGEMENT ==="
input double KeepPrincipal   = 100.0;
input double WithdrawRatio   = 0.50;
input bool   EnableWithdraw  = true;
input bool   EnableReplenish = true;
input bool   EnableMonthlyRpt= true;
...
int      gMagic         = 20240203;    // V3 magic
string   gComment       = "NBG_TW";   // TwoWay tag
...
int OnInit()
{
   ...
   if(cntB > 0) { gCycleLotBuy  = GetFirstLot(POSITION_TYPE_BUY);  Print(StringFormat("[NBG_TW] Resumed BUY: %d pos | lot=%.2f",  cntB, gCycleLotBuy));  }
   if(cntS > 0) { gCycleLotSell = GetFirstLot(POSITION_TYPE_SELL); Print(StringFormat("[NBG_TW] Resumed SELL: %d pos | lot=%.2f", cntS, gCycleLotSell)); }

   FolderCreate("NBG", FILE_COMMON);
   EventSetTimer(60);

   Print(StringFormat("[NBG_TW] Init OK | TWO-WAY GRID | Balance=%.2f | GridStep=%.2f | TP=%.0f%% | ForceClose=%dmin | NoNew=%dmin",
...
void OnDeinit(const int reason)
{
   ...
   Print(StringFormat("[NBG_TW] Deinit reason=%d | Cycles=%d | TotalPnl=%+.2f | Fund=%.2f",
...
void OnTick()
{
   ...
   if(balance < gStartBalance * StopBotPct / 100.0)
   {
      ...
      gBotHalted = true;
      Alert(StringFormat("[NBG_TW] HALTED — balance %.2f below %.0f%% of start %.2f",
   ...
   // EOD force-close (both sides)
   CheckEndOfDay();

   // TP check (each side independent)
   CheckTakeProfit();
   ...
   if(spread > MaxSpread_Pts) return;
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(lparam == 87)
   {
      ...
      Print(StringFormat("[NBG_TW] Withdraw toggled: %s", gWithdrawOn?"ON":"OFF"));
   }
   else if(lparam == 82)
   {
      gReplenishOn = !gReplenishOn;
      Print(StringFormat("[NBG_TW] Replenish toggled: %s", gReplenishOn?"ON":"OFF"));
   }
}

//+------------------------------------------------------------------+
//| EOD — đóng CẢ HAI side trước khi hết phiên                     |
//+------------------------------------------------------------------+
void CheckEndOfDay()
{
   ...
   Print(StringFormat("[NBG_TW] EOD FORCE CLOSE | FloatBUY=%+.2f FloatSELL=%+.2f Total=%+.2f | BuyPos=%d SellPos=%d | %s",
...
void CheckTakeProfit()
{
   ...
   if(cntBuy > 0)
   {
      ...
      if(floatBuy >= target)
      {
         Print(StringFormat("[NBG_TW] TP BUY | Float=%+.2f >= Target=%.2f | Pos=%d",
               floatBuy, target, cntBuy));
   ...
   if(cntSell > 0)
   {
      ...
      if(floatSell >= target)
      {
         Print(StringFormat("[NBG_TW] TP SELL | Float=%+.2f >= Target=%.2f | Pos=%d",
               floatSell, target, cntSell));
...
//+------------------------------------------------------------------+
//| GRID — chạy BUY và SELL song song                               |
//+------------------------------------------------------------------+
void RunGrid()
{
   ...
   // ── BUY side ──────────────────────────────────────────────────
   int cntBuy = CountBuyPositions();
   bool buySkip = (cntBuy == 0 && gLastCloseBuy > 0 && TimeCurrent() - gLastCloseBuy < 10);
   if(!buySkip)
   {
      if(cntBuy == 0)
      {
         ...
         if(VerboseLog)
            Print(StringFormat("[NBG_TW] NEW BUY CYCLE | Lot=%.2f | Bid=%.3f", gCycleLotBuy, bid));
         OpenBuy(1);
      }
      else if(cntBuy < MaxPositions)
      {
         ...
         {
            if(VerboseLog)
               Print(StringFormat("[NBG_TW] GRID ADD BUY #%d | bid=%.3f | lowest=%.3f | drop=%.3f",
   ...
   // ── SELL side ─────────────────────────────────────────────────
   int cntSell = CountSellPositions();
   bool sellSkip = (cntSell == 0 && gLastCloseSell > 0 && TimeCurrent() - gLastCloseSell < 10);
   if(!sellSkip)
   {
      if(cntSell == 0)
      {
         ...
         if(VerboseLog)
            Print(StringFormat("[NBG_TW] NEW SELL CYCLE | Lot=%.2f | Ask=%.3f", gCycleLotSell, ask));
         OpenSell(1);
      }
      else if(cntSell < MaxPositions)
      {
         ...
         {
            if(VerboseLog)
               Print(StringFormat("[NBG_TW] GRID ADD SELL #%d | ask=%.3f | highest=%.3f | rise=%.3f",
...
void OpenBuy(int posNum)
{
   ...
   if(ok)
      Print(StringFormat("[NBG_TW] BUY #%d | Lot=%.2f | Ask=%.3f | Spread=%d",
            posNum, gCycleLotBuy, ask, (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD)));
   else
   {
      Print(StringFormat("[NBG_TW] BUY FAILED #%d | err=%d", posNum, GetLastError()));
...
void OpenSell(int posNum)
{
   ...
   if(ok)
      Print(StringFormat("[NBG_TW] SELL #%d | Lot=%.2f | Bid=%.3f | Spread=%d",
            posNum, gCycleLotSell, bid, (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD)));
   else
   {
      Print(StringFormat("[NBG_TW] SELL FAILED #%d | err=%d", posNum, GetLastError()));
...
{
   Print(StringFormat("[NBG_TW] CloseSide(%s): %s", side==POSITION_TYPE_BUY?"BUY":"SELL", reason));
   ...
   Print(StringFormat("[NBG_TW] CloseSide(%s) OK", side==POSITION_TYPE_BUY?"BUY":"SELL"));
}

//+------------------------------------------------------------------+
//| Đóng TẤT CẢ lệnh (dùng cho DD stop, halt, EOD)                 |
//+------------------------------------------------------------------+
void CloseAll(string reason)
{
   Print("[NBG_TW] CloseAll: " + reason);
   for(int attempt = 1; attempt <= 5; attempt++)
   {
      ulong tickets[]; int n = 0;
      for(int i = PositionsTotal()-1; i >= 0; i--)
...
//+------------------------------------------------------------------+
//| TOTAL FLOATING PNL (cả hai side — dùng cho DD check)           |
//+------------------------------------------------------------------+
...
void CheckNewDay()
{
   ...
   Print(StringFormat("[NBG_TW] === NEW DAY %04d-%02d-%02d | Balance=%.2f | Fund=%.2f ===",
...
void CheckDailyWithdraw()
{
   ...
   if(CountBuyPositions() > 0 || CountSellPositions() > 0) return;  // chờ đóng hết
   ...
   Print(StringFormat("[NBG_TW] DAILY WITHDRAW +%.2f | Bal=%.2f | Fund=%.2f", amount, balance, gWithdrawFund));
   Alert(StringFormat("[NBG_TW] RÚT LÃI NGÀY $%.2f", amount));
   SaveState();
}

//+------------------------------------------------------------------+
//| RÚT CUỐI TUẦN (Friday)                                          |
//+------------------------------------------------------------------+
void CheckWeeklyWithdraw()
{
   ...
   Print(StringFormat("[NBG_TW] WEEKLY WITHDRAW +%.2f | Fund=%.2f", amount, gWithdrawFund));
   Alert(StringFormat("[NBG_TW] RÚT CUỐI TUẦN $%.2f", amount));
   LogWeeklySummary();
   SaveState();
}
...
void CheckWeeklyReplenish()
{
   ...
   Print(StringFormat("[NBG_TW] REPLENISH +%.2f | Fund=%.2f", replenish, gWithdrawFund));
   Alert(StringFormat("[NBG_TW] BÙ VỐN $%.2f", replenish));
   SaveState();
}
...
//| LOG CYCLE                                                        |
//+------------------------------------------------------------------+
void LogCycleSummary(double pnl, bool win, string dir)
{
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   ...
   string msg = StringFormat(
      ...
      gTotalCycles, dir, win?"WIN":"LOSS", pnl, balance, allWR,
      gWinCycles, gLossCycles, gTotalCycles, gMthPnl);
   Print("[NBG_TW] === " + msg + " ===");
...
void LogPerformance()
{
   ...
   Print(StringFormat("[NBG_TW] HOUR | Bal=%.2f Eq=%.2f DD=%.2f%% | BUY:%d(%+.2f) SELL:%d(%+.2f) | Cyc=%d WR=%.1f%% | Fund=%.2f",
         bal, eq, ddNow, cntB, fltB, cntS, fltS, gTotalCycles, allWR, gWithdrawFund));
}
...
void LogWeeklySummary()
{
   ...
   string msg = StringFormat(
      "WEEK %d | %s | PnL=%+.2f | Balance=%.2f | WR=%.1f%% (W=%d L=%d T=%d)",
      gWeekNum, win?"WIN":"LOSS", gWeekPnl, balance, weekWR,
      gWeekWin, gWeekLoss, gWeekCycles);
   Print("[NBG_TW] === " + msg + " ===");
...
{
   double balance  = AccountInfoDouble(ACCOUNT_BALANCE);
   double mthDDpct = gMthHighEq > 0 ? gMthMaxDD / gMthHighEq * 100.0 : 0.0;
   ...
   WriteStatsLog("MONTHLY", StringFormat("MONTHLY REPORT  %04d-%02d  [TWO-WAY GRID]", year, mon));
   WriteStatsLog("MONTHLY", D);
   ...
   WriteStatsLog("MONTHLY", StringFormat("Cycles thang    : %d  (W=%d L=%d  WR=%.1f%%)", gMthCycles, gMthWin, gMthLoss, mthWR));
   WriteStatsLog("MONTHLY", StringFormat("Max DD thang    : %.2f USD (%.1f%%)", gMthMaxDD, mthDDpct));
   WriteStatsLog("MONTHLY", StringFormat("Total PnL       : %+.2f  Cycles=%d  WR=%.1f%%", gTotalPnl, gTotalCycles, allWR));
   WriteStatsLog("MONTHLY", D);
   Print(StringFormat("[NBG_TW] === MONTHLY REPORT %04d-%02d written ===", year, mon));
}
...
void WriteFinalStatsReport()
{
   ...
   Print(StringFormat("[NBG_TW] === FINAL | Bal=%.2f Eq=%.2f | Cyc=%d WR=%.1f%% | Pnl=%+.2f | NetW=%+.2f ===",
...
void SaveState()
{
   ...
   GlobalVariableSet(p+"MonW",      gMonthlyW);
   GlobalVariableSet(p+"MonR",      gMonthlyR);
...
void LoadState()
{
   ...
   if(GlobalVariableCheck(p+"MonW"))      gMonthlyW          = GlobalVariableGet(p+"MonW");
   if(GlobalVariableCheck(p+"MonR"))      gMonthlyR          = GlobalVariableGet(p+"MonR");
...
void WriteStatsLog(string tag, string msg)
{
   ...
   if(gIsTester)
   {
      fname = StringFormat("NBGTW_STATS_%04d%02d.log", yr, mon);
      flags = FILE_WRITE|FILE_READ|FILE_TXT|FILE_ANSI;
   }
   else
   {
      fname = StringFormat("NBG\\NBGTW_STATS_%04d%02d.log", yr, mon);
